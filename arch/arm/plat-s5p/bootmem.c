...
void s5p_reserve_bootmem(struct s5p_media_device *mdevs, int nr_mdevs)
{
	struct s5p_media_device *mdev;
	int i;
	static unsigned long reserved_size=0;
	...
	for (i = 0; i < nr_media_devs; i++) {
		...
		reserved_size += mdev->memsize;
		printk(KERN_INFO "s5pv210: %lx bytes system memory reserved "
			"for %s at 0x%08x total 0x%08lX\n", (unsigned long) mdev->memsize,
			mdev->name, mdev->paddr, reserved_size);
	}
}
