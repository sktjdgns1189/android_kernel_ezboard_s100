...
u32 falinux_io_read_u32( u32 addr )
{
	volatile u32 *base;
			 u32 offset;
			 u32 data;

	base    = ioremap( addr&(~0xFFF) , PAGE_SIZE );
	offset  = (addr & 0x0000FFF)/4;
	data = base[offset];
	iounmap(base);
...
void falinux_io_write_u32( u32 addr, u32 data )
{
	volatile u32 *base;
			 u32 offset;
...
static irqreturn_t s5p_tick_timer_interrupt(int irq, void *dev_id)
{
	...
	//__msg_tick++;
	//if ( 0 == (__msg_tick%1000) ) printk( "..timer heart (%d)\n", tick_timer_mode );
...
#define PADDR_VIC0INTENCLEAR 	0xF2000014
#define PADDR_VIC0SOFTINTCLEAR	0xF200001C

#define INT_TIMER0 				(1<<21)

	falinux_io_write_u32( PADDR_VIC0INTENCLEAR, 	INT_TIMER0 );
	falinux_io_write_u32( PADDR_VIC0SOFTINTCLEAR, 	INT_TIMER0 );
