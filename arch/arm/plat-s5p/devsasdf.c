...
#define CONFIG_FB_S3C_VIRTUAL		y
#define CONFIG_FB_S3C_X_VRES		800
#define CONFIG_FB_S3C_Y_VRES		480
...
void __init s3cfb_set_platdata(struct s3c_platform_fb *pd)
{
	...
	if (!npd)
		 ...
		 * window */
		pmem_start = s5p_get_media_memory_bank(S5P_MDEV_FIMD, 0);
		for (i = 0; i < num_overlay_win; i++) {
...
void __init s3c_fimc0_set_platdata(struct s3c_platform_fimc *pd)
{
	...
	if (!npd)
		...
		npd->pmem_start = s5p_get_media_memory_bank(S5P_MDEV_FIMC0, 0);
		/* size of memory region */
		npd->pmem_size = s5p_get_media_memsize_bank(S5P_MDEV_FIMC0, 0);
...
void __init s3c_fimc1_set_platdata(struct s3c_platform_fimc *pd)
{
	...
	if (!npd)
		...
		/* starting physical address of memory region */
		npd->pmem_start = s5p_get_media_memory_bank(S5P_MDEV_FIMC1, 0);
		/* size of memory region */
		npd->pmem_size = s5p_get_media_memsize_bank(S5P_MDEV_FIMC1, 0);
...
void __init s3c_fimc2_set_platdata(struct s3c_platform_fimc *pd)
{
	...
	if (!npd)
		...
		/* starting physical address of memory region */
		npd->pmem_start = s5p_get_media_memory_bank(S5P_MDEV_FIMC2, 0);
		/* size of memory region */
		npd->pmem_size = s5p_get_media_memsize_bank(S5P_MDEV_FIMC2, 0);
