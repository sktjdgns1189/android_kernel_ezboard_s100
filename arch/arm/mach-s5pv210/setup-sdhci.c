...
#if defined(CONFIG_MACH_EZS5PV210) 	// @FG
unsigned int universal_sdhci0_detect_ext_cd(void)
{
...
/*
printk( "-----%s(%d)\n", __func__,__LINE__ );
	err = gpio_request_one( S5PV210_GPG0(2), GPIOF_IN, "mmc-cd0" );
	if ( err )
	{
		printk( "..err gpio_request_one() S5PV210_GPG0(2)=%d\n", S5PV210_GPG0(2) );	
		return 0;	
	}
		
	card_status = gpio_get_value(S5PV210_GPG0(2));
	gpio_free( S5PV210_GPG0(2) );
*/

	card_status = ( __raw_readl( S5PV210_GPG0_BASE+0x4 ) & (1<<2) ); 
...
#if defined(CONFIG_MACH_EZS5PV210) 	// @FG
	.host_caps	   = MMC_CAP_NEEDS_POLL | MMC_CAP_4_BIT_DATA | MMC_CAP_SD_HIGHSPEED,
	.detect_ext_cd = universal_sdhci0_detect_ext_cd,
#endif
...
//#if defined(CONFIG_MACH_EZS5PV210) 	// @FG
//	hsmmc0_platdata.detect_ext_cd = universal_sdhci0_detect_ext_cd;
//#endif
