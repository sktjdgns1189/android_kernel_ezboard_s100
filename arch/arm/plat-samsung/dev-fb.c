...
void __init s3c_fb_set_platdata(struct s3c_fb_platdata *pd, int num_platdata)
#else
...
#ifdef CONFIG_ARCH_FALINUX
	size = sizeof(struct s3c_fb_platdata) * num_platdata;
#else
	size = sizeof(struct s3c_fb_platdata);
#endif
...
#ifdef CONFIG_ARCH_FALINUX
	npd = kmemdup(pd, size, GFP_KERNEL);
#else
