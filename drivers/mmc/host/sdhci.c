...
static u32 mmc_rx_callcnt = 0;
static u32 mmc_tx_callcnt = 0;
static u32 mmc_rx_blkcnt  = 0;
static u32 mmc_tx_blkcnt  = 0;

//static u32 mmc_tick_buffer[20];
//static u32 mmc_tick_idx = 0;

static mmc_access_info_t  mmc_ainfo[20];
static u32 mmc_aidx = 0;
#endif
#endif
	...
	struct mmc_data *data)
{
	...
	{
		mmc_rx_callcnt ++;
		mmc_rx_blkcnt  += data->blksz*data->blksz;
		if ( 0 == (mmc_rx_callcnt%20) ) 
		{
			printk( "mmc rxcall %d  rxblkcnt=%d\n", mmc_rx_callcnt, mmc_rx_blkcnt );
			{
				int lp;
				for(lp=0; lp<20; lp++)
				{
					printk( "%c%08x:%d\t", mmc_ainfo[lp].ctype, mmc_ainfo[lp].addr, mmc_ainfo[lp].tick );
					if ( 4 == (lp%5) ) printk( "\n" );
				}
				//printk( "\n" );
			}
	...
	{
		mmc_tx_callcnt ++;
		mmc_tx_blkcnt  += data->blksz*data->blksz;
		//if ( 0 == (mmc_tx_callcnt%20) ) printk( "mmc txcall %d  txblkcnt=%d\n", mmc_tx_callcnt, mmc_tx_blkcnt );
	}
	
	mmc_ainfo[mmc_aidx].ctype = (data->flags & MMC_DATA_READ) ? 'r':'w';
	mmc_ainfo[mmc_aidx].tick  = jiffies;
	mmc_ainfo[mmc_aidx].addr  = host->cmd->arg >> 10;
	mmc_ainfo[mmc_aidx].count =data->blksz*data->blksz;
#endif
#endif
}
...
#ifdef CONFIG_FALINUX_ZEROBOOT
#if MMC_ZB_DEBUG
				mmc_ainfo[mmc_aidx].tick  = jiffies - mmc_ainfo[mmc_aidx].tick;
				mmc_aidx = (mmc_aidx+1)%20;
#endif				
#endif
			}
		}
