...
#define CONFIG_S3CTS_ANDROID 1

static int pointercal[7] = { 0,0,0,0,0,0,0 };
static int ANDROID_TS_RESOLUTION_X;
static int ANDROID_TS_RESOLUTION_Y;
...
static struct s3c_ts_info   *last;
static int                  last_flag = 0;
static int                  delta = 30*30;
...
// 좌표를 얻기전에 0 으로 컴파일 한 후 tslib 를 이용해 좌표를 얻어낸다.
// 얻어낸 좌표는 /etc/pointercal 에 생성되며, 본 소스의 600 라인 에 위차한 좌표 배열에 적용 한 후 다시 1 로 변경 후 컴파일 해야 한다.
#if 1
			xtemp = ts->xp;
			ytemp = ts->yp;
...
//	input_set_capability( ts->dev, EV_KEY, BTN_TOUCH );
	...
	ts->dev->phys = ts->phys;
	ts->dev->id.bustype = BUS_HOST;		// BUS_RS232
...
static int __init s3c_ts_init(void)
{
	...
	ANDROID_TS_RESOLUTION_X = 800;
	ANDROID_TS_RESOLUTION_Y = 480;
	

	pointercal[0] = -3398; pointercal[1] =   13; pointercal[2] = 53968096;
    pointercal[3] =     3; pointercal[4] = 2185; pointercal[5] = -2367536;	
	pointercal[6] = 65536;

/* Tmoney
	pointercal[0] =  11;  pointercal[1] = -3417;  pointercal[2] =  54221560;
	pointercal[3] = 2176;  pointercal[4] = 	-22;  pointercal[5] = -2059704;   pointercal[6] = 65536;
*/
