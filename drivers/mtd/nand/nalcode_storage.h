...
extern void nalcode_stroage_read_4k_page( u32 nand_page, u8 *mem_buf );

//-----------------------------------------------------------------------------
#else									// zbi.h 파일의 내용중 발췌된 것이다
														
#define ZEROBOOT_MAGIC					(0xFA0000B2)
#define ZBI_HEADER_VADDR				(0xC0008000)
...
static inline int zb_storage_is_dirty( u8 mark_type, int page )
{
	...
	if ( BUF_STORAGE_ACCESS == STORAGE_ACCESS_MARK_NALCODE ) // NAND_ACCESS_MARK_NALCODE
	{
		ZEROBOOT_DBG( " ..nand dirty type=%d [%02x] [%08x] page=%d\n", mark_type, BUF_STORAGE_ACCESS, ZEROBOOT_MAGIC, page );
		//if ( BUF_ZEROBOOT_MAGIC == ZEROBOOT_MAGIC ) printk ( " ..nand dirty type=%d [%02x] [%08x] page=%d\n", mark_type, BUF_STORAGE_ACCESS, ZEROBOOT_MAGIC, page );
		rtn = -1;
