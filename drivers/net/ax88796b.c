...
// #ifdef CONFIG_MACH_EZS3C6410
	...
	#define CONFIG_AX88796B_8BIT_WIDE			0
	#define CONFIG_AX88796B_EEPROM_READ_WRITE	0
#endif // CONFIG_MACH_EZS3C6410



// [FALINUX] ë¶í¸ë¡ëìì ëê²¨ ë°ë MAC ì£¼ì ì¤ì 
...
ax88796b_load_macaddr (struct net_device *ndev, unsigned char *pMac)
{
	...
	// [FALINUX] - BootLoader ìì ë§¥ì£¼ìë¥¼ ê°ì ¸ ì¨ë¤.
...
/*
 * ----------------------------------------------------------------------------
 * Function Name: ax88796b_receive
 * Purpose:
 * ----------------------------------------------------------------------------
 */
static void ax88796b_receive (struct net_device *ndev)
{
    struct ax_device *ax_local = ax_get_priv (ndev);
	void *ax_base = ax_local->membase;
...
#endif
{
	...
	while (++int_serviced < 12) {

		if ((interrupts = readb (ax_base + EN0_ISR)) == 0)
			break;

		PRINTK (INT_MSG, PFX " isr = 0x%02x\n", interrupts);
...
static int ax88796b_open (struct net_device *ndev)
{
...
#if 1
	{
		...
		//arch/arm/mach-s5pv210/include/mach/regs-gpio.h:39:#define S5PV210_GPH1_BASE             (S5P_VA_GPIO + 0xC20)

		//  Port Group GPH1 Control Register (GPH1CON, R/W, Address = 0xE020_0C20) 
 		gph1_con_val = __raw_readl(S5PV210_GPH1_BASE);
		gph1_con_val |= 0xf00;
 		__raw_writel(gph1_con_val, S5PV210_GPH1_BASE);
	}
#endif
