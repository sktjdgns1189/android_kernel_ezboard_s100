...
static int wait_snapshot = 0;
static DECLARE_WAIT_QUEUE_HEAD(zb_waitq);
void zb_power_wakeup( void )
{
	wake_up_interruptible( &zb_waitq );
}
EXPORT_SYMBOL( zb_power_wakeup );

int __init setup_waitfor_snapshot( char *s )
{
	wait_snapshot = simple_strtoul( s, NULL, 0 );
	return 0;	
}
__setup( "wait_snapshot=", setup_waitfor_snapshot );
#endif
...
void request_suspend_state(suspend_state_t new_state)
{
	...
	if ( 0 < wait_snapshot )
	{
		printk( "=================%s(%d) pname=%s  wait_snapshot=%d-sec\n", __func__,__LINE__, current->comm, wait_snapshot );
		interruptible_sleep_on_timeout( &zb_waitq, wait_snapshot*HZ );
		wait_snapshot = 0;
		printk( "=================%s(%d) pname=%s   wakeup\n",   __func__,__LINE__, current->comm );
	}
#endif
